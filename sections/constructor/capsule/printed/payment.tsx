...
import { physicalAddressee as $physicalAddressee } from "@/schema/addressee";
import z from "zod";
...
export default function Order() {
  ...
  const parsedAddressees = z.array($physicalAddressee).safeParse(addressees);
  ...
  async function onPayment({ intent, method, payee }: { intent: number; method: "OTP" | "credits"; payee: string }) {
    ...
    return await order({
      ...
      product: "capsule",
      body: {
        uploads: uploads.filter((file) => !!file),
        message,
        illustration,
      },
      delivery,
  ...
  return (
    <section className="bg-constructor-gray relative min-h-[calc(100svh-98px)] overflow-hidden pb-64">
      ...
      <div className="px-32">
        <Payment.Root product="capsule" variant="printed" quantity={addressees.length}>
